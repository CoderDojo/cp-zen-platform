...
exports.register = function (server, options, next) {
  ...
  server.route([{
    ...
    method: 'GET',
    path: options.basePath + '/dojos/export-users/{id}',
    handler: exportUserListHandlerNeedsDojoAdmin,
    config: {
  ...
  function exportUserListHandlerNeedsDojoAdmin (request, reply) {
    ...
    request.seneca.act(msg, function (err, resp) {
      ...
      var dojoId = request.params.id;
      request.seneca.act({role: 'cd-dojo', cmd: 'exportUserList', dojoId: dojoId}, function (err, obj) {
        if (err) return reply('Fatal error: ' + err).code(500);
        reply(obj.data).header('Content-Type', 'application/csv').header('Content-Disposition', 'attachment; filename=dojo-'+dojoId+'-user-list.csv');
      });
    });
  }
