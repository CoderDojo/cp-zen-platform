  ...
  function doAct (request, reply, cmd, param, user, type) {
    ...
    if (_.includes(covered, msg.role)) {
      checkPerms(request, msg, function (err, response) {
        if (err) {
          request.log(['error', '403'], {payload: request.payload, params: request.params, url: request.url, user: request.user, error: response}, Date.now());
          return reply(null).code(403);
        }
        if (response && response.allowed && _.isBoolean(response.allowed)) {
          callAct(request, reply, msg, type);
        } else {
          request.log(['error', '40x'], {payload: request.payload, params: request.params, url: request.url, user: request.user, error: response}, Date.now());
          return reply(null).code(403); // Even if it's a 500, we hide our validator is broken, sshhhh :D
        }
      });
    } else {
      callAct(request, reply, msg, type);
