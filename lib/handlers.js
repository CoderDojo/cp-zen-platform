...
module.exports = function(server, role) {
  ...
  var actHandlerNeedsCdfAdminOrIsOwn = function (cmd, param) {
    return actHandlerNeedsUser(cmd, param, {checkCdfAdmin: true, isOwn: true});
  }
    ...
    return function (request, reply) {
      ...
      if (opts && opts.isOwn === true){
        ...
        if(user.user.id !== requestedId){
          // Note: a 200 status code is still returned here (should be 403 by right)
          return reply({ok: false, why: 'You cannot access other user data'});
        }
      }
  ...
  return {
    ...
    actHandlerNeedsCdfAdminOrIsOwn: actHandlerNeedsCdfAdminOrIsOwn
  };
};