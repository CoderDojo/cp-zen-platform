...
module.exports = ({ methods }, role) => {
  function checkPerms({ seneca }, act, callback) {
    ...
    seneca.act(msg, callback);
  }

  function doAct(request, reply, cmd, param, user, type, msgDefault) {
    let msg = { cmd, role, locality: methods.locality(request) };
    ...
    if (_.includes(covered, msg.role)) {
      checkPerms(request, msg, (err, response) => {
        if (err) {
  ...
  function callAct({ seneca }, reply, msg, type) {
    seneca.act(msg, (err, resp) => {
      ...
      if (type && ['csv', 'xml'].indexOf(type) > -1) {
        ...
        if (type === 'xml') {
          reply(resp.data).header('Content-Type', 'application/xml').header('Content-Disposition');
        }
      } else {
  ...
  const actHandler = (cmd, param, type, msgDefault) => (request, reply) => {
  ...
  const actHandlerNeedsUser = (cmd, param, opts, msgDefault) => (request, reply) => {
    ...
    if (opts && opts.isOwn === true) {
      ...
      if (user.user.id !== requestedId) {
        ...
        return reply({
          ok: false,
          why: 'You do not have sufficient permissions to access this feature',
        });
      }
    }
  ...
  const actHandlerNeedsCdfAdmin = (cmd, param, msgDefault) =>
    actHandlerNeedsUser(cmd, param, { checkCdfAdmin: true }, msgDefault);

  // Check if the user is a dojo Admin OR a cdfAdmin
  const actHandlerNeedsDojoAdmin = (cmd, param, type, msgDefault) => (request, reply) => {
    // Note: request.user is set in onPostAuthHandler
    const user = request.user;
    ...
    let msg = {
      ...
      role: 'cd-dojos',
      locality: methods.locality(request),
    };
    ...
    request.seneca.act(msg, (err, { userIsDojoAdmin }) => {
      if (err) return reply(err).code(500);
      ...
      if (!userIsDojoAdmin && !isCDFAdmin) {
        code = 401;
        return reply({
          ok: false,
          why: 'You must be a dojo admin or ticketing admin to access this data',
        }).code(code);
      }
