...
module.exports = function(server, role) {
  ...
  function checkPerms (request, act, callback) {
    ...
    var allowed = false;
    var msg = _.pick(act, ['role','zenHostname','locality','user']);
    msg.act = act.cmd;
  ...
  function doAct (request, reply, cmd, param, user, type) {
    ...
    //  NOTE: while working on it, please add/remove the required libs to check without breaking the front, TODO: remove once finished
    if(msg.role === 'cd-users' || msg.role === 'cd-profiles'){
