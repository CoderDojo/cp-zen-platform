...
module.exports = function (server, role) {
    ...
    if (_.includes(covered, msg.role)) {
      checkPerms(request, msg, function (err, response) {
        if (err) {
          ...
          request.log(['error', '50x'], {status: '403', payload: request.payload, params: request.params, url: request.url, user: request.user, error: response}, Date.now());
          return reply(null).code(403);
        }
        if (response && response.allowed && _.isBoolean(response.allowed)) {
          callAct(request, reply, msg, type);
        } else {
          request.log(['error', '40x'], {status: '403', payload: request.payload, params: request.params, url: request.url, user: request.user, error: response}, Date.now());
          return reply(null).code(403);
        }
      });
    } else {
  ...
  var actHandler = function (cmd, param, type) {
    return function (request, reply) {
      doAct(request, reply, cmd, param, null, type);
    };
  };
  ...
  var actHandlerNeedsUser = function (cmd, param, opts) {
    return function (request, reply) {
      ...
      doAct(request, reply, cmd, param, user, opts && opts.type ? opts.type : null);
    };
  };
