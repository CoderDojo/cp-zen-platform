...
module.exports = function () {
  var joiValidator = {
    ...
    venue: function (required) {
      var valid = {
        isFound: Joi.boolean(),
        alpha2: Joi.string().max(2),
        alpha3: Joi.string().max(3),
      ...
      if (required) {
        //  'state',  'country',
        schema = schema.requiredKeys('address1', 'city', 'type', 'corporate');
      } else {
        //  'state',  'country',
        schema = schema.optionalKeys('address1', 'city', 'type', 'corporate');
      }
      return schema;
    },
    team: function () {
      return Joi.object().keys({
        status: Joi.boolean(),
        mentorsSrc: Joi.array(Joi.string()),
        alternativeMentorsSrc: Joi.string(),
        isValid: Joi.boolean().required()
      });
    },
    charter: function () {
