  ...
  var joiValidator = {
    ...
    dojoLead: function (required) {
      var valid = {
        ...
        frequency: Joi.string().only(['1/w', '2/w', '1/m', '2/m']),
        day: Joi.number().only([1, 2, 3, 4, 5, 6]),
        hour: Joi.string(),
      ...
      var schema = Joi.object().keys(valid);
      var keys = ['name', 'firstSession', 'frequency', 'day', 'hour', 'notes'];
      if (required) {
        keys.push('id');
        keys.push('isValid');
    ...
    venue: function (required) {
      ...
      var schema = Joi.object().keys(valid);
      var keys = ['isFound', 'country', 'geoPoint', 'type', 'address1', 'corporate', 'country', 'place', 'capacity'];
