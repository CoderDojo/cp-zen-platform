...
module.exports = function (options) {
  ...
  seneca.add({ role: plugin, cmd: 'list_my_dojos'}, cmd_list_my_dojos);
  seneca.add({ role: plugin, cmd: 'list_my_dojos_count'}, cmd_list_my_dojos_count);
  ...
  function cmd_list_my_dojos(args, done) {
    //TO DO: query cd/dojos by using id instead of name.
    var seneca = this;
    var dojos  = [];
    seneca.make$('sys/user').load$(args.id, function(err, response) {
      if(err) return done(err);
      var user = response;
      async.each(user.dojos, function(dojo, dojoCb) {
        seneca.make$(ENTITY_NS).load$({name:dojo}, function(err, response) {
          if(err) return dojoCb(err);
          dojos.push(response);
          dojoCb();
        });
      }, function() {
  ...
  function cmd_list_my_dojos_count(args, done) {
    //TO DO: query cd/dojos by using id instead of name.
    var seneca = this;
    var dojos  = [];
    seneca.make$('sys/user').load$(args.user.id, function(err, response) {
      if(err) return done(err);
      var user = response;
      async.each(user.dojos, function(dojo, dojoCb) {
        seneca.make$(ENTITY_NS).load$({name:dojo}, function(err, response) {
  ...
  seneca.act({ role: 'web', use: {
    ...
    map: {
      ...
      'list_my_dojos': {GET: true, alias: 'dojos/:id'},
      'list_my_dojos_count':  {POST: true, alias: 'dojos/count'},
