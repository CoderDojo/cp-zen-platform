...
exports.register = (server, args, next) => {
    ...
    return (request, reply) => {
      ...
      request.seneca.act(msg, (err, out) => {
        ...
        if (out.ok) {
          ...
          request.auth.session.set({ token: out.login.token, target });
        }
        reply(out);
      });
    };
  ...
  function handleRegister({ payload, seneca }, reply) {
    const msg = _.defaults({ role: 'cd-users', cmd: 'register' }, payload);
   ...
   */
  function actHandlerAwardBadge({ payload, seneca }, reply) {
    ...
    const msg = _.defaults({ role: 'cd-users', cmd: 'award_lms_badge' }, JSON.parse(payload));
    seneca.act(msg, (err, resp) => {
