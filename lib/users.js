...
exports.register = function (server, options, next) {
  ...
  server.route([{
    ...
    handler: handlers.actHandlerNeedsUser('get_lms_link'),
    config: {
    ...
    handler: actHandlerCheckCertificate,
    config: {
   ...
   */
  function actHandlerCheckCertificate (request, reply) {
    var certif = request.payload;
    var signature = certif.header.signature;
    delete certif.header.signature;
    var stringCertif = JSON.stringify(certif);

    var checkValMsg = {role: 'cd-users', cmd: 'check_lms_certificate_authenticity',
     certif: stringCertif, signature: signature};
    request.seneca.act(checkValMsg, function (err, resp) {
      if (err) return reply(err).code(500);
      if (!resp.ok) return reply().code(403);

      var msg = _.defaults({role: 'cd-users', cmd: 'award_lms_badge'}, request.payload);
      request.seneca.act(msg, function (err, resp) {
        if (err) return reply(err).code(500);
        reply(resp).code(200);
      });
