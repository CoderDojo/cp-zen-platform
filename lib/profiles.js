...
const defaultImage = new Buffer(
  fs.readFileSync(path.join(__dirname, '/../web/public/img/avatars/avatar.png'), 'base64'),
  'base64',
);
...
exports.register = (server, args, next) => {
  ...
  const handlers = Handlers(server, 'cd-profiles');
  ...
  function handleAvatarImage(request, reply) {
    ...
    const msg = _.defaults(
      { user, role: 'cd-profiles', cmd: 'get_avatar', id: request.params.id },
      request.payload,
    );
    request.seneca.act(msg, (err, res) => {
      ...
      const buf = new Buffer(res.imageData, 'base64');
