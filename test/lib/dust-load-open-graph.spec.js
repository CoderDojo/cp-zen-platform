...
lab.describe('dust-load-open-graph', () => {
  ...
  lab.test(
    'should apply multiple same opengraph tag based on context',
    done => {
      const chunk = {
        write: sinon.spy(),
      };
  ...
  lab.test(
    'should apply multiple different opengraph tags based on context',
    done => {
      const chunk = {
        write: sinon.spy(),
      };
      const context = {
        stack: {
          head: { context: { preload: { title: ['a', 'b'], doubidou: 'a' } } },
        },
      };
      fn(chunk, context);
      expect(chunk.write).to.have.been.calledWith(
        '<meta property="og:title" content="a"/><meta property="og:title" content="b"/><meta property="og:doubidou" content="a"/>'
      );
      done();
    }
  );
