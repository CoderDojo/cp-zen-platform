(function() {
  ...
  function manageDojosCtrl($scope, alertService, auth, tableUtils, cdDojoService, $location, cdCountriesService, cdUsersService) {
    ...
    function allSigned(dojo) {
      ...
      _.each(creators, function(creator) {
        var result = _.findWhere(agreements, {
    ...
    $scope.loadPage = function(filter, resetFlag, cb) {
      ...
      if ($scope.filter.usersDojos && $scope.filter.usersDojos.length > 0) {
        var idsFilter = {
          ids: {
            'values': $scope.filter.usersDojos
          }
        };
      ...
      cdDojoService.search(filteredQuery).then(function(result) {
        $scope.dojos = _.map(result.records, function(dojo) {
          dojo.verified = _.findWhere(verificationStates, {
            value: dojo.verified
          });
          return dojo;
        });
  ...
  angular.module('cpZenPlatform')
    .controller('manage-dojo-controller', ['$scope', 'alertService', 'auth',
