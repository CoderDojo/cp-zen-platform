angular.module("cpZenPlatform").factory("dojoUtils", [
  "$location",
  "$translate",
  "$state",
  "$rootScope",
  ...
  function(
    ...
    const approvalRequired = ["mentor", "champion"];

    dojoUtils.requestToJoin = function(requestInvite, dojoId) {
      if (!requestInvite.userType) {
        window.alert("Error");
      } else {
        const userType = requestInvite.userType.name;
        ...
        auth.get_loggedin_user(
          user => {
            ...
            const query = { userId: user.id, dojoId };
            cdDojoService.getUsersDojos(query, response => {
              if (_.isEmpty(response)) {
                if (_.includes(approvalRequired, userType)) {
                  cdDojoService.requestInvite(data, response => {
                    usSpinnerService.stop("dojo-detail-spinner");
                    if (!response.error) {
                      alertService.showAlert(
                        $translate.instant("Join Request Sent")
                      );
                    } else {
                      alertService.showError(
                        $translate.instant(response.error)
                      );
                    }
                  });
                } else {
                  ...
                  cdDojoService.saveUsersDojos(userDojo, response => {
                    usSpinnerService.stop("dojo-detail-spinner");
                    $state.go($state.current, $state.params, { reload: true });
                    alertService.showAlert(
                      $translate.instant("Successfully Joined Dojo")
                    );
                  });
                }
              }
    ...
    dojoUtils.getDojoURL = function(dojo) {
      if (dojo) {
        const urlSlug = dojo.url_slug || dojo.urlSlug;
        return `/dojo/${urlSlug}`;
      }
    };
    ...
    dojoUtils.isHavingPerm = function(user, dojoId, perm, userDojo) {
      function checkUserDojo(userDojo) {
        if (!userDojo || userDojo.length < 1) {
          return deferred.reject();
        }

        const isHavingPerm = _.find(
          userDojo[0].userPermissions,
          userPermission => userPermission.name === perm
        );
      ...
      if (user && !_.isEmpty(user)) {
        const isCDF = _.includes(user.roles, "cdf-admin");
        const query = { userId: user.id, dojoId, deleted: 0 };
        const isHavingPerm = _.includes(user.roles, perm);
        if (isHavingPerm || isCDF) {
          deferred.resolve(isHavingPerm || isCDF);
        } else if (userDojo) {
          checkUserDojo([userDojo]);
        } else {
          cdDojoService.getUsersDojos(query, checkUserDojo, err => {
            deferred.reject(err);
          });
        }
      } else {
    ...
    dojoUtils.getFrequencyStrings = function() {
      const strings = {};
      strings.frequencies = [
        { id: "1/w", name: $translate.instant("Weekly") },
        { id: "2/m", name: $translate.instant("Every two weeks") },
        { id: "1/m", name: $translate.instant("Monthly") },
        { id: "other", name: $translate.instant("Other") }
      ];
    ...
    dojoUtils.startingDojoSrcs = [
      { id: "search_engine", name: $translate.instant("Search Engine") },
      { id: "volunteers", name: $translate.instant("CoderDojo Volunteers") },
      {
        id: "foundation",
        name: $translate.instant(
          "Raspberry Pi Foundation / CoderDojo Foundation outreach"
        )
      },
      { id: "social", name: $translate.instant("Social media") },
      { id: "future_learn", name: "Future Learn" },
      {
        id: "employer",
      ...
      { id: "library", name: $translate.instant("Library or youth group") },
      { id: "word_of_mouth", name: $translate.instant("Word of Mouth") },
      { id: "other", name: $translate.instant("Other (comment field)") }
    ];
    ...
    dojoUtils.nextDateOptions = [
      {
        id: "already-did",
        name: $translate.instant("We already had a session")
      },
      {
        id: "next-3-months",
        name: $translate.instant("In the next three months")
      },
