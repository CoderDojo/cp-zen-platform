(function() {
  ...
  var resolveDojo = function($q, $stateParams, cdDojoService) {
    ...
    if ($stateParams.legacyId && _.isNumber(parseInt($stateParams.legacyId))) {
      cdDojoService.list(
        { mysqlDojoId: parseInt($stateParams.legacyId) },
        function(data) {
          dfd.resolve(data[0]);
        },
        function(err) {
      ...
      ($stateParams.dojoId && !_.isNumber($stateParams.dojoId)) ||
      ...
      $stateParams.country &&
      $stateParams.path &&
      _.isString($stateParams.country) &&
      _.isString($stateParams.path)
    ) {
      cdDojoService.find(
        {
  ...
  var resolves = {
    profile: function($stateParams, cdUsersService, $q) {
      if ($stateParams.userId) {
        return cdUsersService
          .userProfileDataPromise({ userId: $stateParams.userId })
          .then(winCb, failCb);
      } else {
        return $q.resolve({
          data: {
    ...
    ownProfile: function(auth, cdUsersService) {
      return auth.get_loggedin_user_promise().then(function(currentUser) {
        if (currentUser) {
          return cdUsersService
            .userProfileDataPromise({ userId: currentUser.id })
            .then(winCb, failCb);
        } else {
          winCb(void 0);
        }
    ...
    usersDojos: function(auth, cdDojoService) {
      return auth.get_loggedin_user_promise().then(function(currentUser) {
        if (currentUser) {
          return cdDojoService
            .getUsersDojosPromise({ userId: currentUser.id })
            .then(winCb, failCb);
        } else {
          winCb(void 0);
        }
    ...
    championsForUser: function($stateParams, cdUsersService, $q) {
      if ($stateParams.userId) {
        return cdUsersService
          .loadChampionsForUserPromise($stateParams.userId)
          .then(winCb, failCb);
      } else {
        return $q.resolve({
          data: [],
    ...
    championsForLoggedInUser: function($stateParams, cdUsersService, auth) {
      return auth.get_loggedin_user_promise().then(function(currentUser) {
        if (currentUser) {
          return cdUsersService
            .loadChampionsForUserPromise(currentUser.id)
            .then(winCb, failCb);
        } else {
          winCb(void 0);
        }
    ...
    parentsForLoggedInUser: function($stateParams, cdUsersService, auth) {
      return auth.get_loggedin_user_promise().then(function(currentUser) {
        if (currentUser) {
          return cdUsersService
            .loadParentsForUserPromise(currentUser.id)
            .then(winCb, failCb);
        } else {
          winCb(void 0);
        }
    ...
    dojoAdminsForUser: function($stateParams, cdUsersService, $q) {
      if ($stateParams.userId) {
        return cdUsersService
          .loadDojoAdminsForUserPromise($stateParams.userId)
          .then(winCb, failCb);
      } else {
        return $q.resolve({
          data: [],
    ...
    dojoAdminsForLoggedInUser: function($stateParams, cdUsersService, auth) {
      return auth.get_loggedin_user_promise().then(function(currentUser) {
        if (currentUser) {
          return cdUsersService
            .loadDojoAdminsForUserPromise(currentUser.id)
            .then(winCb, failCb);
        } else {
          winCb(void 0);
        }
    ...
    event: function($stateParams, cdEventsService) {
      return cdEventsService
        .loadPromise($stateParams.eventId)
        .then(winCb, failCb);
    },
    sessions: function($stateParams, cdEventsService) {
      return cdEventsService
        .searchSessionsPromise({
  ...
  angular
      ...
      function(
        ...
        $stateProvider
          ...
          .state('manage-dojo-users', {
            ...
            template:
              '<cd-manage-dojo-users class="row"></cd-manage-dojo-users>',
            controller: [
              'initUserTypes',
              'currentUser',
              '$scope',
          ...
          .state('manage-dojo-active-users', {
            ...
            template:
              '<cd-dojo-manage-active-users></cd-dojo-manage-active-users>',
            params: {
          ...
          .state('manage-dojo-pending-users', {
            ...
            parent: 'manage-dojo-users',
            template:
              '<cd-dojo-manage-pending-users init-user-types="initUserTypes"></cd-dojo-manage-pending-users>',
            params: {
          ...
          .state('edit-dojo-eventbrite', {
            ...
            parent: 'dashboard',
            template:
              '<cd-eventbrite-integration class="col-xs-12"></cd-eventbrite-integration>',
            params: {
              pageTitle: 'Edit Dojo',
            },
          })
          ...
          .state('embedded.event', {
            ...
            url: '/event/:eventId',
            template:
              '<div class="cd-event-list col-xs-12">' +
            ...
            controller: function(
              ...
              cdDojoService.load($scope.event.dojoId).then(function(dojo) {
          ...
          .state('start-dojo', {
            ...
            url: '/start-dojo?id',
            template:
              '<cd-start-dojo current-user="currentUser"></cd-start-dojo>',
            params: {
              referer: 'start-dojo',
            ...
            controller: function(currentUser, $scope, $state) {
              if (!currentUser.data) {
                var next = $state.href('login', {
                  referer: $state.href('start-dojo', $state.current.params),
                });
                return window.location.assign(next);
              }
              $scope.currentUser = currentUser.data;
              return $state.go('start-dojo.champion', $state.current.params, {
                replace: true,
              });
            },
          })
          ...
          .state('my-children.add', {
            url: '/add',
            template:
              '<cd-add-child parent-profile-data="parentProfileData" class="cd-sidebar__content--padded"></cd-add-child>',
            controller: [
              '$scope',
              'ownProfile',
              function($scope, ownProfile) {
          ...
          .state('my-children.child', {
            url: '/:id',
            template:
              '<cd-child-card parent-profile-data="parentProfileData"></cd-child-card>',
            controller: [
              '$scope',
              'ownProfile',
              function($scope, ownProfile) {
        ...
        $urlRouterProvider.when('/register?referer', [
          '$state',
          '$location',
    ...
    .config([
      ...
      function($translateProvider, $cookiesProvider) {
        ...
        $translateProvider
          .useUrlLoader('/locale/data?format=mf')
          .useCookieStorage()
    ...
    .config([
      ...
      function(tmhDynamicLocaleProvider) {
        tmhDynamicLocaleProvider.localeLocationPattern(
          '/components/angular-i18n/angular-locale_{{locale}}.js'
        );
      },
    ])
    .config([
              ...
              $delegate[0].compile = function() {
                ...
                return function postLink(scope, element, attr) {
                  element.bind(action, { attrs: attr }, function(event) {
                    var data = !_.isUndefined(event.data.attrs['data-name'])
                      ? event.data.attrs['data-name']
                      : !_.isUndefined(event.data.attrs['aria-label'])
                      ? event.data.attrs['aria-label']
                      : !_.isEmpty(event.target.name)
                      ? event.target.name
    ...
    .run([
      ...
      function($window, $cookieStore, tmhDynamicLocale, Analytics, $translate) {
        ...
        var userLocality =
          (
            $cookieStore.get('NG_TRANSLATE_LANG_KEY') ||
            $translate.proposedLanguage()
          ).replace(/%22/g, '') || 'en_US';
        var userLangCode = userLocality ? userLocality.split('_')[0] : 'en';
        googleCaptchaScriptTag.setAttribute(
          'src',
        ...
        var momentLocale = userLocality
          ? userLocality.replace('_', '-').toLowerCase()
          : 'en-us';
    ...
    .run([
      ...
      function(
        ...
        $translate.instant = function(key) {
          var translation = originalTranslateInstant.apply(
            $translate,
            arguments
          );
          if (!translation) {
        ...
        $rootScope.$on('$stateChangeStart', function(
          ...
          if (
            !$cookieStore.get('verifyProfileComplete') &&
            toState.parent === 'dashboard'
          ) {
            if (toState.name !== 'edit-user-profile') {
              verifyProfileComplete().then(
                ...
                function(err) {
                  if (err.message !== 'User not found.') {
                    alertService.showError(
                      $translate.instant(
                        'An error has occured verifying your profile.'
                      )
                    );
                  }
                }
        ...
        var cookieAcceptListener = $rootScope.$on(
          '$stateChangeSuccess',
        ...
        var cookieDisclaimerListener = $rootScope.$on(
          '$viewContentLoaded',
          function() {
            jQuery('body').cookieDisclaimer({
              text: $translate.instant(
                "By using this website you agree to the use of cookies. You can read about our cookie policy <a href='/privacy-statement#cookies'>here</a>."
              ),
    ...
    .factory('verifyProfileComplete', [
      'cdUsersService',
      'auth',
      '$q',
      function(cdUsersService, auth, $q) {
        return function() {
          var deferred = $q.defer();
          auth.get_loggedin_user_promise().then(
            function(user) {
              if (user && user.id) {
                cdUsersService.userProfileDataPromise({ userId: user.id }).then(
                  function(profile) {
                    deferred.resolve({
                      complete: profile.requiredFieldsComplete,
                      userId: user.id,
                    });
                  },
                  function(err) {
    ...
    .controller('cdDashboardCtrl', [
      ...
      'Idle',
      'auth',
      function($scope, $uibModal, $cookieStore, $window, Idle, auth) {
