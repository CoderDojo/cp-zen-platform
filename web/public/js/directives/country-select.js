(function () {
  ...
  function countrySelect($parse) {
    ...
    return {
      restrict: 'A',
      require: 'ngModel',
      template: '<select><option value="" disabled>Select a country</option><option>' + countries.join('</option><option>') + '</option></select>',
      replace: true,
      link: function (scope, elem, attrs, ngModelController) {
       elem.bind('change', function (e) {
          ngModelController.$setViewValue(elem.val());
        });

        scope.$watch(ngModelController.$viewValue, function (country) {
          elem.val(country);
        });
      }
    };
  ...
  angular.module('cdCountrySelect', [])
