;(function() {
  ...
  angular
    ...
    .directive('cdDojoManageActiveUsers', function () {
      return {
        restrict: 'AE',
        templateUrl: '/directives/tpl/dojo/manage-active-users',
        controller:
        ['$scope', '$state', '$q', 'cdDojoService', 'alertService', 'tableUtils', 'usSpinnerService',
        'cdBadgesService', '$translate', 'utilsService', 'cdEventsService', 'permissionService', '$timeout',
        '$ngBootbox', 'userUtils', 'cdChooseRoleModal', 'translationKeys' , 'cdUsersService',
        function ($scope, $state, $q, cdDojoService, alertService, tableUtils, usSpinnerService,
          cdBadgesService, $translate, utilsService, cdEventsService, permissionService,
          ...
          $scope.loadPage = function(resetFlag) {
            ...
            function getInviteUserTypes(done) {
              cdDojoService.getUsersDojos({userId: user.id, dojoId: dojoId, deleted: 0}, function (usersDojos) {
                ...
                var inviteUserTypes = angular.copy($scope.initUserTypes.data);
              ...
              cdDojoService.loadDojoUsers({dojoId:dojoId, limit$: $scope.pagination.itemsPerPage, skip$: loadPageData.skip, sort$: $scope.queryModel.sort, userType: $scope.queryModel.userType, name: $scope.queryModel.name}, function (response) {
                ...
                _.each(response, function (user) {
                  ...
                  user.frontEndTypes = _.map(thisUsersDojoLink.userTypes, function (userType) {
                    var userTypeFound = _.find($scope.initUserTypes.data, function (initUserType) {
          ...
          $scope.loadUserTypes = function(query) {
            var filteredUserTypes = _.chain($scope.initUserTypes.data)
              ...
              if(hasPermission) {
                ...
                var updatedUserTypes = _.chain(angular.copy(user.frontEndTypes))
                  ...
                  .map(function (userType) {
                    ...
                    var initUserTypeFound = _.find($scope.initUserTypes.data, function (initUserType) {
