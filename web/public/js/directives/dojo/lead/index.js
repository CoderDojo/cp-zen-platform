'use strict';
angular.module('cpZenPlatform').component('cdDojoLead', {
  bindings: {
    id: '<'
  },
  restrict: 'EA',
  ...
  controller: [
    ...
    function(
      ...
      ctrl.getReferenceValue = function(id) {
        return _.find(dojoUtils.startingDojoSrcsWithDeprecated, { id: id })
          .name;
      };
      ...
      function getDojo() {
        if (ctrl.lead.application.dojo) {
          return cdDojoService
            .load(ctrl.lead.application.dojo.id)
            .then(function(res) {
              ctrl.dojo = res.data;
            })
            .then(function() {
              return cdDojoService.getAvatar(ctrl.dojo.id).then(function(res) {
      ...
      function getCharter() {
        if (ctrl.lead.application.charter && ctrl.lead.application.charter.id) {
          return cdAgreementsService
            .load(ctrl.lead.application.charter.id)
            .then(function(res) {
      ...
      function getProfile() {
        return (
          cdUsersService
            .userProfileData({ userId: ctrl.lead.userId })
            ...
            .then(function() {
              ctrl.otherDojos = [];
              var dojos = _.omitBy(ctrl.profile.dojos, function(dojo) {
                return dojo.id === ctrl.dojo.id;
              });
              // Lookup previous experience, even if deleted
              if (_.keys(dojos).length) {
                return cdDojoService
                  .getUsersDojos({
                    dojoId: { in$: _.map(dojos, 'id') },
                    userId: ctrl.lead.userId
                  })
                  .then(function(res) {
                    var userDojos = res.data;
                    var setUserDojoRole = function(dojo) {
                      var userDojo = _.find(userDojos, function(uD) {
                        return dojo.id === uD.dojoId;
                      });
                      ctrl.otherDojos.push(
                        userDojo.userTypes.join(', ') + ' @ ' + dojo.name
                      );
                    };
