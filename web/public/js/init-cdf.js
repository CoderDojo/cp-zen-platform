(function() {
  ...
  var resolves = {
    ...
    agreement: function(cdAgreementsService, $stateParams, $window, auth) {
      return auth.get_loggedin_user_promise().then(function(user) {
        return cdAgreementsService
          .loadUserAgreementPromise(user.id)
          .then(winCb, failCb);
      });
    },
  ...
  angular
    ...
    .run([
      ...
      '$cookieStore',
      'tmhDynamicLocale',
      function($window, $cookieStore, tmhDynamicLocale) {
        ...
        var userLangCode = userLocality
          ? userLocality.replace(/%22/g, '').split('_')[0]
          : 'en';
