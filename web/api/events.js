...
exports.register = function(server, eOptions, next) {
  ...
  server.route([
        ...
        validate: {
          payload: {
            eventInfo: Joi.object({
              ...
              notifyOnPublish: Joi.boolean().allow(null),
    ...
    },
    {
      ...
      handler: handlers.actHandlerNeedsUser('list', 'eventId', null, {
        ctrl: 'applications',
      }),
      config: {
        auth: auth.apiUser,
        description:
          "Load user's applications (and its children) for this event",
        tags: ['api', 'events'],
        validate: {
          params: {
            eventId: Joi.string()
    ...
    },
    {
      ...
      handler: handlers.actHandlerNeedsUser('deleteApplication', [
        'eventId',
        'applicationId',
      ]),
      config: {
    ...
    },
    {
      ...
      path: `${
        options.basePath
      }/events/export-guest-list/dojo/{dojoId}/event/{eventId}/{status}-export.csv`,
      handler: handlers.actHandlerNeedsUser(
        'exportGuestList',
        ['dojoId', 'eventId', 'status'],
        { type: 'csv' }
      ),
      config: {
        auth: auth.apiUser,
            ...
            applications: Joi.array().items(
              Joi.object({
                ...
                parentEmailSubject: Joi.object({
                  approved: Joi.string()
                    .valid('A ticket has been booked for your child for %1$s')
                    .valid('A ticket has been booked for your child for %1$s'),
                  pending: Joi.string()
                    .valid(
                      'Your childs ticket request for %1$s is pending approval'
                    )
                    .valid(
                      'Your childs ticket status for %1$s has been changed to pending'
                    ),
                  cancelled: Joi.string()
                    .valid(
                      'A ticket for your child for %1$s has been cancelled'
                    )
                    .optional(),
                }),
                emailSubject: Joi.object({
                  received: Joi.string().valid(
                    'Your ticket request for %1$s has been received'
                  ),
                  approved: Joi.string()
                    .valid('Your ticket for %1$s has been booked')
                    .valid('Your ticket request for %1$s has been approved'),
                  pending: Joi.string()
                    .valid('Your ticket request for %1$s is pending approval')
                    .valid(
                      'Your ticket status for %1$s has been changed to pending'
                    ),
                  cancelled: Joi.string()
                    .valid('Your ticket request for %1$s has been cancelled')
                    .optional(),
                }),
                dojoEmailSubject: Joi.object({
                  approved: Joi.string().valid(
                    'A ticket has been booked for %1$s'
                  ),
                  pending: Joi.string().valid(
                    'A ticket request has been made for %1$s'
                  ),
                }),
                eventId: Joi.string()
                  .guid()
    ...
    },
    {
      ...
      path: `${
        options.basePath
      }/events/ticket/{ticketId}/invitations/user/{userId}`,
          ...
          payload: {
            invitation: {
              ...
              emailSubject: Joi.object({
                ...
                approved: Joi.string().valid(
                  'Your ticket for %1$s has been booked'
                ),
                pending: Joi.string().valid(
                  'Your ticket request for %1$s is pending approval'
                ),
              }),
            },
          },
