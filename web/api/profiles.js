...
const defaultImage = Buffer.from(
  fs.readFileSync(
    path.join(__dirname, '/../public/img/avatars/avatar.png'),
    'base64'
  ),
  'base64'
);
...
exports.register = function(server, eOptions, next) {
  ...
  function handleAvatarImage(request, reply) {
    ...
    request.seneca.act(msg, (err, res) => {
      if (err || !res) {
        ...
        return reply(defaultImage)
          .header('Content-Type', 'image/png')
          .header('Content-Length', defaultImage.length);
      }
  ...
  server.route([
    ...
    },
    {
      ...
      handler: handlers.actHandlerNeedsUser('load_user_profile', 'userId', {
        isOwn: true,
      }),
      config: {
        auth: auth.apiUser,
